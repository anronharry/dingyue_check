...
class SessionLogger:
    ...
    def log_node_test(
        self, filename: str, total_nodes: int, survived_nodes: int, deleted: bool = False
    ):
        """
        记录一次节点真实连通性测速的结果
        ...
        detail = (
            f"已自动归档（全部节点失效）"
            if deleted
            else f"幸存节点: {survived_nodes}/{total_nodes}"
        )
        self.records.append(
            {
                "type": "node_test",
    ...
    def log_sub_check(
        self, filename: str, total_links: int, valid_links: int, deleted: bool = False
    ):
        """
        记录一次订阅地址有效性检测的结果
        ...
        """
        detail = (
            f"已自动归档（无有效链接）" if deleted else f"有效链接: {valid_links}/{total_links}"
        )
        self.records.append(
            {
                "type": "sub_check",
    ...
    def print_session_summary(self):
        ...
        print(
            f"  {Fore.YELLOW}🕐 会话开始:{Style.RESET_ALL} {self.session_start.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print(
            f"  {Fore.YELLOW}🕑 会话结束:{Style.RESET_ALL} {session_end.strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def _save_log_file(
        self, session_end: datetime, duration: float, node_count: int, sub_count: int
    ):
        """将会话日志保存为精美排版的文本文件"""
        try:
            ...
            with open(log_filename, "a", encoding="utf-8") as f:
                ...
                for idx, rec in enumerate(self.records, 1):
                    ...
                    status_icon = (
                        "❌"
                        if rec.get("deleted") or rec["survived"] == 0
                        else "✅"
                        if rec["survived"] / max(1, rec["total"]) > 0.5
                        else "⚠️"
                    )
                    f.write(
                        f"[{idx:02d}] {icon} [{type_str}] [{rec['time']}] 文件: {rec['file']}\n"
                    )
    ...
    def _auto_clean_old_logs(self):
        ...
        try:
            ...
            if removed:
                print(
                    f"{Fore.YELLOW}🗑️  已自动清理 {removed} 个超过 {keep_days} 天的旧日志文件{Style.RESET_ALL}"
                )
        except Exception as e:
