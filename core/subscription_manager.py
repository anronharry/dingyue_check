...
async def async_fetch_nodes_from_subscriptions(
    target_file: str, client_session: aiohttp.ClientSession = None, status_callback=None
) -> Tuple[List[ProxyNode], List[str], List[str]]:
    """
            ...
            try:
                ...
                if node_lines:
                    return {
                        "status": "valid",
                        "url": url,
                        "nodes": [("raw", l) for l in node_lines],
                    }
                    ...
                    if isinstance(data, dict) and "proxies" in data:
                        ...
                        if len(clash_nodes) > 0:
                            return {
                                "status": "valid",
                                "url": url,
                                "nodes": [("clash", n) for n in clash_nodes],
                            }
    ...
    print(
        f"{Fore.GREEN}✅ 共解析到 {len(all_nodes)} 个节点（来自 {len(valid_urls)} 个有效订阅）{Style.RESET_ALL}"
    )

    return all_nodes, invalid_urls, valid_urls


def fetch_nodes_from_subscriptions(
    target_file: str, http_session=None
) -> Tuple[List[ProxyNode], List[str], List[str]]:
    """
