...
def _print_batch_summary(batch_summary: list) -> None:
    ...
    for rec in batch_summary:
        ...
        table.add_row(f"{color}{ratio}[/]", f"{color}{status}[/]", fname)
    print()
    console.print(table)
    print()
...
def _dedup_and_rename(nodes: list[dict]) -> list[dict]:
    ...
    if dup_count > 0:
        print(
            f"{Fore.YELLOW}🔄 去重: 移除 {dup_count} 个重复节点，剩余 {len(deduped)} 个唯一节点{Style.RESET_ALL}"
        )
    used_names: set = set()
...
async def _handle_url_file(
    target_file: str, sub_session: aiohttp.ClientSession, sub_clean_policy: str, batch_summary: list
) -> None:
    """
    ...
    if not final_nodes:
        ...
        if invalid_sub_urls:
            ...
            if new_path:
                print(
                    f"{Fore.YELLOW}📂 全部订阅均失效，文件已移入 Old_file: {os.path.basename(new_path)}{Style.RESET_ALL}"
                )
    ...
    if invalid_sub_urls and os.path.exists(target_file):
        ...
        if do_clean:
            try:
                ...
                print(
                    f"{Fore.GREEN}✅ 已从文件中移除 {len(invalid_sub_urls)} 个失效订阅 URL。{Style.RESET_ALL}"
                )
...
async def _process_single_file(ctx: _NodeTestContext) -> None:
    ...
    print(
        f"  [阶段] 节点准备完成，共 {len(final_nodes)} 个，正在加载 {engine.engine_name} 内核配置..."
    )
    ...
    tasks = [
        asyncio.create_task(
            engine.async_test_node(n["name"], _timeout_ms, _test_url, ctx.test_session, sem)
        )
    ...
    if len(valid_results) == 0:
        ...
        if new_path:
            ...
            batch_summary.append(
                {"file": fname, "total": total, "valid": 0, "status": "全失效归档"}
            )
        else:
    ...
    if not invalid_names:
        ...
        batch_summary.append(
            {"file": fname, "total": total, "valid": len(valid_results), "status": "全通过"}
        )
        return
    ...
    if do_clean:
        ...
        try:
            ...
            print(
                f"{Fore.GREEN}✅ 成功移除了无效节点，保留 {len(survived)} 个已验证节点！{Style.RESET_ALL}"
            )
            get_logger().log_node_test(fname, total, len(survived))
            batch_summary.append(
                {"file": fname, "total": total, "valid": len(survived), "status": "已清理"}
            )
        except (OSError, ValueError) as e:
            print(f"{Fore.RED}❌ 清理时出错: {e}{Style.RESET_ALL}")
            get_logger().log_node_test(fname, total, len(valid_results))
            batch_summary.append(
                {"file": fname, "total": total, "valid": len(valid_results), "status": "清理出错"}
            )
    else:
        print("已取消清理，文件保持原样。")
        get_logger().log_node_test(fname, total, len(valid_results))
        batch_summary.append(
            {"file": fname, "total": total, "valid": len(valid_results), "status": "保持原样"}
        )


def run_node_latency_test(
    target_files,
    mode: str = "auto",
    clean_policy: str = "no",
    export_policy: str = "no",
    sub_clean_policy: str = "no",
    ...
    if sys.platform == "win32":
        ...
        asyncio.run(
            _async_run_node_latency_test(
                target_files, mode, clean_policy, export_policy, sub_clean_policy, status_callback
            )
        )


async def _async_run_node_latency_test(
    target_files,
    mode: str = "auto",
    clean_policy: str = "no",
    export_policy: str = "no",
    ...
    try:
        async with (
            aiohttp.ClientSession(connector=sub_conn, timeout=sub_timeout) as sub_session,
            ...
            for target_file in target_files:
                ...
                if file_mode == "url":
                    await _handle_url_file(
                        target_file, sub_session, sub_clean_policy, batch_summary
                    )
                    continue
                ...
                ok = (
                    converter.parse_txt_file(target_file)
                    if ext == ".txt"
                    else converter.parse_yaml_file(target_file)
                    if ext in (".yaml", ".yml")
                    else False
                )
                if not ok or not converter.nodes:
                    print(
                        f"{Fore.RED}❌ 未从文件中解析到有效的 Vmess/SS/Trojan 节点{Style.RESET_ALL}"
                    )
                    new_path = archive_to_old_file(target_file, "无法解析")
                    if new_path:
                        print(
                            f"{Fore.YELLOW}📂 文件已移入 Old_file: {os.path.basename(new_path)}{Style.RESET_ALL}"
                        )
                        get_logger().log_node_test(
                            os.path.basename(target_file), 0, 0, deleted=True
                        )
                    else:
                        get_logger().log_node_test(
                            os.path.basename(target_file), 0, 0, deleted=False
                        )
                    continue
