...
class ExportCacheTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.result = {
            ...
            "_normalized_nodes": [
                {"name": "node1", "protocol": "vmess", "raw": "vmess://abc#node1"}
            ],
        }
    ...
    def test_successful_parse_writes_cache_index(self) -> None:
        self.service.save_subscription_cache(
            owner_uid=1, source="https://example.com/sub", result=self.result
        )
        self.assertEqual(len(self.service.get_index_snapshot()), 1)

    def test_yaml_and_txt_artifacts_are_generated(self) -> None:
        entry = self.service.save_subscription_cache(
            owner_uid=1, source="https://example.com/sub", result=self.result
        )
    ...
    def test_expired_entries_are_cleaned_up(self) -> None:
        ...
        self.service = ExportCacheService(
            index_path=str(index_path), cache_dir=str(self.tmpdir / "cache_exports"), ttl_hours=48
        )
    ...
    def test_yaml_subscription_txt_export_contains_protocol_urls(self) -> None:
        ...
        entry = self.service.save_subscription_cache(
            owner_uid=1, source="https://example.com/yaml", result=yaml_result
        )
