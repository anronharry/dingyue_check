...
from web.aggregate import service as aggregate_service
...
from web.aggregate.node_cleaning import _aggregate_node_key
from web.aggregate.service import _quick_filter_aggregate_nodes
from web.constants import AGG_PUBLISH_SERVER_LIMIT, AGG_PUBLISH_SOURCE_LIMIT
...
async def _disabled_verify(_nodes):
    return [], {"verify_attempted": 0, "verify_alive": 0, "verify_mode": "disabled"}
...
class OwnerAggregateNodeCleaningTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_normalize_aggregate_proxy_rejects_missing_required_fields(self):
        valid = _normalize_aggregate_proxy(
            {"name": "A", "type": "trojan", "server": "1.1.1.1", "port": "443", "password": "p1"}
        )
        self.assertIsNotNone(valid)
        self.assertEqual(valid["port"], 443)
        self.assertIsNone(
            _normalize_aggregate_proxy({"type": "vmess", "server": "1.1.1.1", "port": 443})
        )
        self.assertIsNone(
            _normalize_aggregate_proxy(
                {"type": "ss", "server": "1.1.1.1", "port": 443, "password": "p"}
            )
        )
        self.assertIsNone(
            _normalize_aggregate_proxy(
                {"type": "trojan", "server": "", "port": 443, "password": "p"}
            )
        )
    ...
    async def test_quick_filter_keeps_only_alive_nodes_and_attaches_latency(self):
        ...
        original = aggregate_service._verify_aggregate_nodes_with_mihomo
        aggregate_service._verify_aggregate_nodes_with_mihomo = _disabled_verify
        try:
            filtered, stats = await _quick_filter_aggregate_nodes(runtime, nodes, state=state)
        finally:
            aggregate_service._verify_aggregate_nodes_with_mihomo = original
    ...
    async def test_cached_verified_success_streak_promotes_node_into_stable_pool(self):
        ...
        state = _FakeAggregateState(
            {
                _aggregate_node_cache_key(node): {
                    "mode": "verify",
                    "status": "alive",
